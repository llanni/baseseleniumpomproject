...
public class Utilities {
    ...
    //generate a random email, given a domain as input (e.g. domain: leonardolanni.com)
    public static String generateRandomEmail(String domain) {
        String defaultDomain = domain;
        String randomEmail = "qa.test.automation." + System.currentTimeMillis() + "." + generateRandomPassword(6) + "@" + defaultDomain;
    ...
    /*
    Extract activation link from email or return exception
    ...
    public static String extractActivationLink(String htmlEmailBody) throws Exception{
        ...
        else throw new Exception ("The email does not seem to contain an activation link - provided: " + activationLink);
    }

    /*
    Extract activation code and return it as a link
    //TODO: make it more robust
     */
    ...
    /*
    Given the driver, check if the current page contains a text showing the input element
     */
    ...
    public static boolean verifyUserIsLoggedIn(WebDriver driver, TestConfig config, String userName) throws Exception {
        try{   WebElement verificationPopup = driver.findElement(config.getLocator("registrationpanel.verification.popup.title"));
        ...
        {
            //test on mobile
            return !Utilities.isUIshowingText(driver,config.getData("homepage.register.motivation.message")) && driver.findElement(config.getLocator("homepage.watched.word")).isDisplayed();
        }
        else
        {
            // test on desktop
            return Utilities.isUIshowingText(driver,userName) && driver.findElement(config.getLocator("homepage.watched.word")).isDisplayed();
        }
    }
