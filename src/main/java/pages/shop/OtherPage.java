...
public class OtherPage extends BasePage {
    ...
    public OtherPage(WebDriver driver, TestConfig config) throws Exception {
        ...
        assertThat(backButton.getText().equalsIgnoreCase(config.getData("videopage.back.button.label"))).isTrue().withFailMessage("Unexpected back button label'");
        ...
        assertThat(watchNextBox.getText().equalsIgnoreCase(config.getData("videopage.watch.next.label"))).isTrue().withFailMessage("Unexpected next video label'");
    ...
    public OtherPage clickNextVideoButton() throws Exception {
        ...
        assertThat(oldUrl.equals(newUrl)).isFalse().withFailMessage("It seems that the new video is the same as the old one (same url)");
