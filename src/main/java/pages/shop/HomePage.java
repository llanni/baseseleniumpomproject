...
public class HomePage extends BasePage {
    ...
    //constructor: connect the UI elements to the WebElements via locators (read from prop file).
    ...
    public HomePage(WebDriver driver, TestConfig config) throws Exception {
        ...
        //TODO: improve this -- this is to fix a random failing due probably of some dynamic content not loaded - add a proper wait();
        Utilities.pauseExecution(4000);

        // this.watchContactBanner = driver.findElement(config.getLocator("homepage.watchContactBanner"));
        ...
        assertThat(contactUsSelector.getAttribute("href").trim().toLowerCase().contains(contactUsEmail.toLowerCase())).isTrue().withFailMessage("The contact us email does not point to mailto: " + contactUsEmail);
    }
    ...
    public OtherPage clickOnVideo(int idx) throws Exception {
        ...
        if (idx < 0 || idx >= noVideos)
            throw new Exception("The input video index: " + idx + " must be between 0 and number of videos: " + noVideos);
        ...
        js.executeScript("var evt = document.createEvent('MouseEvents');" + "evt.initMouseEvent('click',true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0,null);" + "arguments[0].dispatchEvent(evt);", video);
        return new OtherPage(driver, config);
    }

    //click on a video containg a text (text normalised) -- if many, return the first;
    ...
    //return the index a video containg a text (text normalised) -- if many, return the first;
    ...
    public String getVideoTitle(int idx) throws Exception {
        ...
        {//maybe lazy loading? move via js to the video and wait some msecs;
            System.out.println("Title for video with idx: " + idx + " seems empty, trying again to grab it...");
            Utilities.pauseExecution(300);
            JavascriptExecutor js = (JavascriptExecutor) driver;
            js.executeScript("var evt = document.createEvent('MouseEvents');" + "evt.initMouseEvent('move',true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0,null);" + "arguments[0].dispatchEvent(evt);", video);
            title = video.getText();
        }
        if (title.equalsIgnoreCase(""))
